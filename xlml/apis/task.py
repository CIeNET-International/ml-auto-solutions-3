...
class Runner(abc.ABC):
  ...
  def wait_workload_reach_to_step(
    ...
    """Block until the workload logs the expected step.

    Implemented on the base runner because it only reads pod logs through the
    GKE API; it does not depend on how the workload was launched.
    """
  ...
  def interrupt_workload(self, is_targeting_on_last_node: bool) -> DAGNode:
    """Delete a node backing the workload to simulate a disruption.

    Implemented on the base runner because it deletes a Compute Engine
    instance directly; it does not depend on how the workload was launched.
    """
...
class BaseRunnerNodeInterruptionTask(BaseRunnerTask):
  """Base task for running workloads that get interrupted mid-run."""
