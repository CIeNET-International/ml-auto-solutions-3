...
from dags.common.vm_resource import Project, TpuVersion
from xlml.apis import metric_config
from xlml.utils import bigquery, mantaray, metric
...
if composer_env.is_prod_env() or composer_env.is_dev_env():
  ...
  # post-process to push test result to BigQuery Database
  # via self-generated metadata
  @task_decorators
  def new_process_metrics(
    ...
    benchmark_id = (
        f"reqeust-rate-{current_request_rate}-vllm-benchmark-tpu-nightly-"
        f"LLaMA3.1-8B-v{local_tpuvm_tpuversion_value}-4"
    )
  ...
  def run_on_v6e_4_persistent_tpuvm(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
      ...
      print(f"Command finished with code: {result.exit_code}")
  ...
  def get_path(current_request_rate: int):
    ...
    output_location = (
        "gs://ml-auto-solutions/output/pytorch-xla/vllm_benchmark_nightly/"
        f"vllm-nightly-v6e-8-{current_datetime}-{current_request_rate}/"
        "metric_report.jsonl"
    )
    return output_location

  # call execute commands for vllm nightly benchmark test result,
  # and push test result to BigQuery Database in post-process
  ...
  def run_vllm_nightly_on_v6e_4_persistent_tpuvm(current_request_rate: int):
    ...
    ### order of execute
    chain(run_on_v6e_4_persistent_tpuvm_func, post_process)
  ...
  print(
      "Skipping creating Mantaray DAGs since not running in Prod "
      "or Dev composer environment."
  )
