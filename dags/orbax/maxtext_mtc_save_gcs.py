...
from dags.orbax.util.validation_util import BaseValidation
  ...
  for mode, image in DOCKER_IMAGES:
    for test_config in test_configs:
      for slice_num in test_config.slices:
        ...
        validation_with_default_pod_config = validation_util.factory_create_validation_config(
        ...
        validate_steps_task = validation_util.validate_log_with_step(validation_with_default_pod_config, start_time=start_time, end_time=end_time)

        # Return a string with the correct bucket path name used in the actual
        # training
        validation_gcs_bucket = validation_util.factory_create_validation_config(
            test_config=test_config,
            namespace="default",
        ...
        validation_with_replicator_pod_config = validation_util.factory_create_validation_config(
            test_config=test_config,
            text_filter="Successful: backup for step",
            namespace="gke-managed-checkpointing",
            gcs_bucket_run_name=validation_gcs_bucket.get_backup_bucket_path() if isinstance(validation_gcs_bucket, validation_util.ValidationPod) else None
        )
        validate_gcs_bucket_task = validation_util.validate_log_with_gcs(validation_with_replicator_pod_config, start_time=start_time, end_time=end_time)
