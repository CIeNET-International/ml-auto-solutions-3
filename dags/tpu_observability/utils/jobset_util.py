...
def calculate_pod_latency(pod_name: str, metadata_map: dict, scheduled_ts: str):
  ...
  # Get Pod creation time from our map
  pod_ts_str = metadata_map.get(pod_name, {}).get("timestamp")
...
def wait_for_tpu_utilized_chips(
  ...
  if len(time_series) > 0:
    latest_value = time_series[0].points[0].value.double_value

    logging.info(
        "Instance %s: Current Utilized Chips = %s (Expected = %s)",
        instance_id,
        latest_value,
        expected_chip_count,
    )

    if latest_value >= expected_chip_count:
      logging.info("Target chip utilization reached and chips are HEALTHY.")
      return True
    else:
      logging.info("Chips detected but not all are utilized yet. Retrying...")
