...
from dags.tpu_observability.utils import node_pool_util
...
class Command:
  ...
  @staticmethod
  def get_credentials_command(node_pool: node_pool_util.Info) -> str:
    """
...
def get_replica_num(
    replica_type: str, job_name: str, node_pool: node_pool_util.Info
) -> int:
  """
  ...
  api_client = gke.get_authenticated_client(
      node_pool_util.project_id,
      node_pool_util.region,
      node_pool_util.cluster_name,
  )
...
def get_running_pods(
    node_pool: node_pool_util.Info, namespace="default"
) -> list[str]:
  """
...
@task
def run_workload(
    node_pool: node_pool_util.Info, yaml_config: str, namespace: str
) -> TimeUtil:
  """
...
@task
def end_workload(
    node_pool: node_pool_util.Info, jobset_name: str, namespace: str
):
  """
...
@task
def get_active_pods(
    node_pool: node_pool_util.Info, namespace: str
) -> list[str]:
  """
  Deletes all JobSets from the GKE cluster to clean up resources.
...
def wait_for_jobset_ttr_to_be_found(node_pool: node_pool_util.Info) -> bool:
  """
...
@task.sensor(poke_interval=30, timeout=600, mode="reschedule")
def wait_for_all_pods_running(num_pods: int, node_pool: node_pool_util.Info):
