...
class SchedulingHelper:
  ...
  registry: dict[XpkClusterConfig, list[Dag]] = {
      XpkClusters.TPU_V5P_128_CLUSTER: [
          Dag(dag_id="test1", dag_run_timeout=dt.timedelta(minutes=120)),
          Dag(dag_id="test2", dag_run_timeout=dt.timedelta(minutes=120)),
          Dag(dag_id="test3", dag_run_timeout=dt.timedelta(minutes=120)),
          Dag(dag_id="test4", dag_run_timeout=dt.timedelta(minutes=120)),
          Dag(dag_id="test5", dag_run_timeout=dt.timedelta(minutes=120)),
          Dag(dag_id="test6", dag_run_timeout=dt.timedelta(minutes=120)),
          Dag(dag_id="test7", dag_run_timeout=dt.timedelta(minutes=120)),
          Dag(dag_id="test8", dag_run_timeout=dt.timedelta(minutes=120)),
          Dag(dag_id="test9", dag_run_timeout=dt.timedelta(minutes=120)),
          Dag(dag_id="test10", dag_run_timeout=dt.timedelta(minutes=120)),
          Dag(dag_id="test11", dag_run_timeout=dt.timedelta(minutes=120)),
          Dag(dag_id="test12", dag_run_timeout=dt.timedelta(minutes=120)),
      ],
  }
  ...
  def ArrangeScheduleTime(
    ...
    """
    Arrange the schedule time for a given DAG in a cluster.
    Args:
      cluster: The XPK cluster configuration.
      dag_id: The DAG ID to arrange the schedule for.
      day_of_week: The days of the week to schedule the DAG.
    Returns:
      A cron expression string representing the scheduled time.
    Raises:
...
if __name__ == "__main__":
  test_case = SchedulingHelper.ArrangeScheduleTime(
      XpkClusters.TPU_V5P_128_CLUSTER, "test1"
  )
